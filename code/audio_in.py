...
class AudioInputProcessor:
            ...
            try:
                ...
                if self.transcription_task and self.transcription_task.done() and not self._transcription_failed:
                    ...
                    if task_exception and not isinstance(task_exception, asyncio.CancelledError):
                        ...
                        logger.error(
                            f"👂🛑 Transcription task finished with unexpected error: {task_exception}. Stopping audio processing.",
                        ...
                        logger.warning(
                            "👂⏹️ Transcription task is no longer running (completed or cancelled). Stopping audio processing."
                        )
    ...
    def shutdown(self) -> None:
        ...
        if self.transcription_task and not self.transcription_task.done():
            task_name = (
                self.transcription_task.get_name()
                if hasattr(self.transcription_task, "get_name")
                else "TranscriptionTask"
            )
