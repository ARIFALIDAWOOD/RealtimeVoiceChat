...
try:
    ...
    if dotenv_spec:
        ...
        # Load .env from the code/ directory (same directory as this module)
        code_dir = os.path.dirname(os.path.abspath(__file__))
        env_path = os.path.join(code_dir, ".env")
        load_dotenv(env_path)
        logger.debug(f"🤖⚙️ Loaded environment variables from .env file at: {env_path}")
    else:
...
OPENAI_BASE_URL = os.getenv("OPENAI_BASE_URL")  # Optional: override default OpenAI API endpoint
...
class LLM:
    ...
    def __init__(
        ...
        # Use explicit base_url argument, or OPENAI_BASE_URL env var, or None (default OpenAI endpoint)
        self.effective_openai_base_url = (
            self._base_url
            if self.backend == "openai" and self._base_url
            else (OPENAI_BASE_URL if self.backend == "openai" else None)
        )
        ...
        with self._client_init_lock:
            ...
            try:
                if self.backend == "openai":
                    if not self.effective_openai_key:
                        logger.error(
                            "🤖💥 OpenAI backend selected but OPENAI_API_KEY is not set! Please set it in code/.env file."
                        )
                        raise ValueError("OPENAI_API_KEY environment variable is required for OpenAI backend")
                    logger.info(
                        f"🤖🔌 Initializing OpenAI client (base_url: {self.effective_openai_base_url or 'default'})..."
                    )
                    ...
                    if self.ollama_session and self.effective_ollama_url:
                        ...
                        if initial_check_ok:
                            ...
                            logger.warning(
                                f"🤖🔌 Initial Ollama connection check failed for {self.effective_ollama_url}. Attempting 'ollama ps' fallback."
                            )
                            if _run_ollama_ps():
                                ...
                                time.sleep(3)
                                second_check_ok = _check_ollama_connection(
                                ...
                                if second_check_ok:
                                    ...
                                    logger.error(
                                        f"🤖💥 Ollama connection check still failed after running 'ollama ps'."
                                    )
                                    init_ok = False  # Explicitly set to false
                            else:
                                # ollama ps command failed or was not found
                                logger.error(
                                    f"🤖💥 'ollama ps' command failed or not found. Cannot verify/start server. Initialization failed for {self.effective_ollama_url}."
                                )
                                init_ok = False  # Explicitly set to false
    ...
    def _register_request(self, request_id: str, request_type: str, stream_obj: Optional[Any]):
        ...
        with self._requests_lock:
            ...
            logger.debug(
                f"🤖ℹ️ Registered active request: {request_id} (Type: {request_type}, Stream: {type(stream_obj)}, Count: {len(self._active_requests)})"
            )
        ...
        while attempts <= max_retries:
            ...
            try:
                ...
                generator = self.generate(
                    text=prompt, history=None, use_system_prompt=True, request_id=prewarm_request_id, temperature=0.1
                )
                ...
                last_error = e
                logger.warning(
                    f"🤖🔥⚠️ Prewarm attempt {attempts + 1}/{max_retries+1} connection/timeout error during generation: {e}"
                )
                if attempts < max_retries:
                    ...
                    logger.error(
                        f"🤖🔥💥 Prewarm failed permanently after {attempts + 1} generation attempts due to connection issues."
                    )
                    return False
                ...
                if isinstance(e, ConnectionError) and "connection failed" in str(e):
                    logger.error(
                        "   (This likely indicates the initial lazy initialization failed its connection check or `ollama ps` fallback)"
                    )
                ...
                logger.debug(
                    f"🤖🔥ℹ️ [{prewarm_request_id}] Prewarm attempt finished. generate()'s finally handles tracking cleanup."
                )
                if generator is not None and hasattr(generator, "close"):
                    try:
                        ...
                        logger.warning(
                            f"🤖🔥⚠️ [{prewarm_request_id}] Error closing generator in prewarm finally: {close_err}",
                            exc_info=False,
                        )
    ...
    def generate(
        ...
        **kwargs: Any,
        ...
        # Catch specific exceptions first
            ...
            logger.error(
                f"🤖💥 Unexpected error in generation pipeline for {req_id}: {e}", exc_info=True
            )  # Log traceback for unexpected
            ...
            with self._requests_lock:
                if req_id in self._active_requests:
                    ...
                    logger.debug(
                        f"🤖🗑️ [{req_id}] Removing request from tracking and attempting stream close in generate's finally block."
                    )
                    ...
                    logger.debug(
                        f"🤖🗑️ [{req_id}] Request already removed from tracking before finally block completion."
                    )
        ...
        try:
            for chunk in stream:
                ...
                with self._requests_lock:
                    if request_id not in self._active_requests:
                        logger.info(
                            f"🤖🗑️ OpenAI/LMStudio stream {request_id} cancelled or finished externally during iteration."
                        )
            ...
            if is_cancelled:
                logger.warning(
                    f"🤖⚠️ OpenAI/LMStudio stream connection error likely due to cancellation for {request_id}: {e}"
                )
            else:
            ...
            if stream and hasattr(stream, "close") and callable(stream.close):
                try:
                    ...
                    logger.warning(
                        f"🤖⚠️ [{request_id}] Error closing OpenAI stream in finally: {close_err}", exc_info=False
                    )
        ...
        try:
                ...
                for chunk_bytes in response.iter_content(chunk_size=None):  # None = read whatever is available
                    ...
                    while "\n" in buffer:
                        ...
                        try:
                            ...
                            logger.error(
                                f"🤖💥 [{request_id}] Error processing Ollama stream chunk: {e}", exc_info=True
                            )
            ...
            # Catch the specific error from the race condition
                ...
                if "'NoneType' object has no attribute 'read'" in str(e):
                    ...
                    if is_cancelled:
                        logger.warning(
                            f"🤖⚠️ [{request_id}] Caught AttributeError ('NoneType' has no attribute 'read') during Ollama stream iteration, likely due to concurrent cancellation. Stopping iteration."
                        )
                    else:
                        ...
                        logger.warning(
                            f"🤖⚠️ [{request_id}] Caught AttributeError ('NoneType' has no attribute 'read') during Ollama stream iteration. Request *might* not be marked cancelled yet, but stopping iteration as stream is likely closed."
                        )
                    # Break the (now non-existent) outer loop implicitly by exiting the 'try' block.
                else:
                    # If it's a different AttributeError, re-raise it.
                    logger.error(
                        f"🤖💥 [{request_id}] Caught unexpected AttributeError during Ollama stream iteration: {e}",
                        exc_info=True,
                    )
                    raise e
            # --- End Change ---
            ...
            if response:
                try:
                    ...
                    logger.warning(
                        f"🤖⚠️ [{request_id}] Error closing Ollama response in finally: {close_err}", exc_info=False
                    )
    ...
    def measure_inference_time(self, num_tokens: int = 10, **kwargs: Any) -> Optional[float]:
        ...
        if not self._lazy_initialize_clients():
            logger.error(
                f"🤖⏱️💥 Measurement failed: Could not initialize backend client/connection for {self.backend}."
            )
            return None
    ...
    def __enter__(self) -> Generator[str, None, None]:
        ...
        try:
            ...
            self.generator = self.llm.generate(
                self.prompt, self.history, self.use_system_prompt, request_id=self.request_id, **self.kwargs
            )
...
if __name__ == "__main__":
    ...
    logging.basicConfig(
        level=main_log_level, format="%(asctime)s - %(name)s - %(levelname)s - %(message)s", stream=sys.stdout
    )
    ...
    if REQUESTS_AVAILABLE:
        try:
            ...
            ollama_llm = LLM(backend="ollama", model=ollama_model_env, system_prompt="You are concise and helpful.")
            ...
            if prewarm_success:
                ...
                try:
                    ...
                    with LLMGenerationContext(
                        ollama_llm, "What is the capital of France? Respond briefly."
                    ) as generator:
