...
class SpeechPipelineManager:
    ...
    def __init__(
        ...
        # Get base URL from environment if using OpenAI backend
        llm_base_url = None
        ...
        self.llm = LLM(
            ...
            no_think=no_think,
            base_url=llm_base_url,  # Pass base_url to LLM constructor
        )

        # Initialize and measure LLM latency, but tolerate missing/unavailable backends
        try:
            self.llm.prewarm()
            self.llm_inference_time = self.llm.measure_inference_time()
            if self.llm_inference_time is None:
                raise ValueError("LLM inference time measurement returned None")
            logger.debug(f"🗣️🧠🕒 LLM inference time: {self.llm_inference_time:.2f}ms")
        except Exception as e:
            logger.error(
                "🗣️🧠💥 Failed to initialize or measure LLM inference time. "
                "Continuing with LLM disabled / unknown latency.",
                exc_info=True,
            )
            # Fallback to 0 so downstream math/logging still works
        ...
        logger.info(
            f"🗣️⏱️ Full output pipeline latency: {self.full_output_pipeline_latency:.2f}ms "
    ...
    def _llm_inference_worker(self):
        ...
        while not self.shutdown_event.is_set():
            ...
            try:
                logger.debug(f"🗣️🧠🔄 [Gen {gen_id}] LLM Worker: Starting iteration over generator...")
                ...
                for chunk in current_gen.llm_generator:
                    ...
                    if token_count == 1:
                        ...
                        logger.info(f"🗣️🧠📝 [Gen {gen_id}] LLM Worker: First chunk received: '{chunk[:50]}...'")
                ...
                if not iterator_started:
                    logger.warning(
                        f"🗣️🧠⚠️ [Gen {gen_id}] LLM Worker: Generator iteration never started (empty generator or immediate failure)."
                    )
                ...
                if not current_gen.llm_aborted and not current_gen.quick_answer_provided:
                    if token_count > 0:
                        ...
                        logger.warning(
                            f"🗣️🧠⚠️ [Gen {gen_id}] LLM Worker: No tokens received and no quick answer provided. This may indicate an API error."
                        )

            except ConnectionError as e:
                logger.error(f"🗣️🧠💥 [Gen {gen_id}] LLM Worker: Connection error during generation: {e}")
                logger.error(f"🗣️🧠💥 [Gen {gen_id}] Check network connectivity, API key, and base URL configuration")
            ...
            if self.running_generation:
                ...
                if self.running_generation.abortion_started:
                    ...
                    self.abort_generation(
                        wait_for_completion=wait_for_finish,
                        timeout=7.0,
    ...
    def process_prepare_generation(self, txt: str):
        ...
        try:
            ...
            logger.error(
                f"🗣️🧠💥 [Gen {new_gen_id}] Connection error creating LLM generator (check API key, network, base URL): {e}"
            )
            logger.error(f"🗣️🧠💥 [Gen {new_gen_id}] Backend: {self.llm_provider}, Model: {self.llm_model}")
            if self.llm_provider == "openai":
                logger.error(f"🗣️🧠💥 [Gen {new_gen_id}] Check OPENAI_API_KEY in .env file and OpenAI API status")
